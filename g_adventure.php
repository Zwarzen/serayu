...
<div class="col-lg-8" style="width: 50%; padding: 0px; ">
  <div class="row">
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
      ...
      if ($pencarian != '') {
        $sql = "SELECT * FROM data WHERE title LIKE '%$pencarian%' AND WHERE adv_id!=0";
        $query = $sql;
        $queryJml = $sql;
      }
    }else {
      $query = "SELECT * FROM data WHERE adv_id!=0 LIMIT $posisi, $batas";
      $queryJml = "SELECT * FROM data WHERE adv_id!=0";
      $no = $posisi + 1;
    }
